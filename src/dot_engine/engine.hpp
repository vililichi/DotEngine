...
class DotEngine {
    ...
    std::vector<std::vector<std::shared_ptr<DotBodyInterface>>> get_collision_search_mapping();
...
void DotEngine::update(const float delta_t)
{
    ...
    {
        const size_t i = i_p_1  - 1;
        std::shared_ptr<DotUniversalLawInterface> universal_law_ptr = m_universal_law_ptrs[i];
    ...
    {
        const size_t i = i_p_1  - 1;
        std::shared_ptr<DotForceInterface> force_ptr = m_force_ptrs[i];
    ...
    for(size_t i = 0; i < m_body_ptrs.size(); i++)
    {
        ...
        {
            std::shared_ptr<DotBodyInterface> body_ptr_j = m_body_ptrs[j];
            ...
            if( info.has_collision )
            {
                    ...
                    {
                        std::shared_ptr<DotCollisionEffectInterface> effet_ptr = m_collision_effect_ptrs[k];
